...
1. **CV-JD Matching** identifies:
   - REQUIRED missing: "Microsoft Excel", "communication", "business processes"
   - PREFERRED missing: "curiosity", "continuous improvement mindset"

2. **Input Recommendation** (_classify_keywords):
   - ✅ Filters out keywords already in CV (8 filtered)
   - ❌ **Does NOT classify based on REQUIRED/PREFERRED**
   - ❌ Just puts everything in Tier 2 placeholder

3. **AI Recommendation Generator**:
   - Receives filtered keywords
   - AI classifies using its own judgment
   - ❌ **AI ignores the REQUIRED flag**
   - Result: REQUIRED keywords go to Tier 2 or wrong category

4. **CV Tailoring**:
   - Only integrates Tier 1 keywords
   - Since Tier 1 is mostly empty, nothing gets integrated!

### What Should Happen

1. **CV-JD Matching**: ✅ (already correct)

2. **Input Recommendation** (_classify_keywords):
   - Should PRE-CLASSIFY:
   - **REQUIRED missing → Tier 1 (automatic)**
   - **PREFERRED missing → Send to AI for Tier 2/3**

3. **AI Recommendation**:
   - Only classifies PREFERRED/OPTIONAL keywords
   - Cannot override Tier 1 (REQUIRED keywords)

4. **CV Tailoring**:
   - Integrates all Tier 1 (all REQUIRED keywords)
...
**In `ats_recommendation_service.py`, update `_classify_keywords` to:**

1. Load CV-JD matching results
2. Get `missed_required_keywords`
3. Filter out keywords already in CV
4. **Pre-populate Tier 1 with REQUIRED missing keywords**
5. Only pass PREFERRED/OPTIONAL to AI for Tier 2/3

This way:
- ✅ All REQUIRED missing keywords → Tier 1 (guaranteed)
- ✅ AI only classifies optional keywords
- ✅ Tier 1 is never empty when there are required gaps
- ✅ CV tailoring always addresses critical gaps first
