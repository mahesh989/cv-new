...
class CVTailoringService:
    ...
    async def tailor_cv_with_validation(
        self,
        request: CVTailoringRequest,
        max_attempts: int = 3
    ) -> CVTailoringResponse:
        """
        Tailor CV with validation and retry mechanism
        Ensures ALL Tier 1 keywords are integrated
        
        CRITICAL FIX: Retry loop for 100% Tier 1 integration
        ...
        Returns:
            CVTailoringResponse with validated tailoring
        """
        logger.info(f"🎯 [PHASE8] Starting CV tailoring with validation (max {max_attempts} attempts)")
        
        # Extract Tier 1 keywords from recommendations
        tier1_keywords = self._extract_tier1_keywords(request.recommendations)
        total_tier1 = sum(len(v) for v in tier1_keywords.values())
        
        logger.info(f"📊 [PHASE8] Tier 1 keywords to integrate: {total_tier1}")
        for category, keywords in tier1_keywords.items():
            if keywords:
                logger.info(f"   - {category}: {keywords}")
        
        best_result = None
        best_integration_rate = 0.0
        
        for attempt in range(1, max_attempts + 1):
            logger.info(f"🔄 [PHASE8] Attempt {attempt}/{max_attempts}")
            
            try:
                # Generate tailored CV
                response = await self.tailor_cv(request)
                
                if not response.success:
                    logger.warning(f"⚠️ [PHASE8] Attempt {attempt} failed to generate CV")
                    continue
                
                # Validate Tier 1 keyword integration
                validation = self._validate_tier1_integration(
                    response.tailored_cv,
                    tier1_keywords
                )
                
                integration_rate = validation["integration_rate"]
                integrated = validation["integrated_keywords"]
                missing = validation["missing_keywords"]
                
                logger.info(f"📊 [PHASE8] Attempt {attempt} results:")
                logger.info(f"   - Integration rate: {integration_rate:.1%}")
                logger.info(f"   - Integrated: {len(integrated)}/{total_tier1}")
                logger.info(f"   - Missing: {len(missing)}")
                
                if missing:
                    logger.warning(f"⚠️ [PHASE8] Missing keywords: {missing}")
                
                # Track best result
                if integration_rate > best_integration_rate:
                    best_integration_rate = integration_rate
                    best_result = response
                
                # Check if 100% integration achieved
                if integration_rate >= 1.0:
                    logger.info(f"✅ [PHASE8] 100% Tier 1 integration achieved!")
                    logger.info(f"=" * 80)
                    logger.info(f"✅ [PHASE8] COMPLETE - Summary:")
                    logger.info(f"   Company: {request.recommendations.company}")
                    logger.info(f"   Total attempts: {attempt}")
                    logger.info(f"   Final integration rate: {integration_rate:.1%}")
                    logger.info(f"   Tier 1 keywords integrated: {len(integrated)}/{total_tier1}")
                    logger.info(f"=" * 80)
                    return response
                
                # If not last attempt, update request with missing keywords
                if attempt < max_attempts:
                    logger.info(f"🔄 [PHASE8] Retrying with focus on missing keywords...")
                    request.custom_instructions = (
                        f"CRITICAL: You MUST integrate these missing Tier 1 keywords: {missing}\n"
                        f"These keywords are essential and must appear naturally in the CV.\n"
                        f"Previous attempt only integrated {len(integrated)}/{total_tier1} keywords."
                    )
            
            except Exception as e:
                logger.error(f"❌ [PHASE8] Attempt {attempt} error: {e}")
                continue
        
        # After all attempts
        if best_result:
            logger.warning(f"⚠️ [PHASE8] Max attempts reached. Best integration: {best_integration_rate:.1%}")
            logger.info(f"=" * 80)
            logger.info(f"⚠️ [PHASE8] COMPLETE (Partial) - Summary:")
            logger.info(f"   Company: {request.recommendations.company}")
            logger.info(f"   Total attempts: {max_attempts}")
            logger.info(f"   Best integration rate: {best_integration_rate:.1%}")
            if best_integration_rate < 1.0:
                missing_count = total_tier1 - int(best_integration_rate * total_tier1)
                logger.warning(f"   ⚠️ Still missing {missing_count} Tier 1 keywords after {max_attempts} attempts")
            logger.info(f"=" * 80)
            return best_result
        else:
            raise Exception(f"Failed to generate tailored CV after {max_attempts} attempts")
    
    def _extract_tier1_keywords(self, recommendations: RecommendationAnalysis) -> Dict[str, List[str]]:
        """
        Extract Tier 1 keywords from recommendations
        
        Args:
            recommendations: RecommendationAnalysis object
            
        Returns:
            Dictionary with technical/soft/domain Tier 1 keywords
        """
        tier1 = recommendations.keyword_integration.get("tier1_integrate_immediately", {})
        
        extracted = {}
        for category in ["technical", "soft", "domain"]:
            keywords = tier1.get(category, [])
            if isinstance(keywords, list):
                ...
                for item in keywords:
                    if isinstance(item, str):
                        ...
                        extracted[category].append(item["keyword"])
        
        return extracted
    
    def _validate_tier1_integration(
        self,
        tailored_cv: TailoredCV,
        tier1_keywords: Dict[str, List[str]]
    ) -> Dict[str, Any]:
        """
        Validate that ALL Tier 1 keywords are present in tailored CV
        
        Args:
            tailored_cv: TailoredCV object
            tier1_keywords: Dict of Tier 1 keywords by category
            
        Returns:
            Validation result dictionary
        """
        # Convert CV to lowercase text for searching
        cv_text = json.dumps(tailored_cv.model_dump()).lower()
        ...
        for category, keywords in tier1_keywords.items():
            for keyword in keywords:
                ...
                if keyword_lower in cv_text:
                    ...
                    logger.warning(f"   ⚠️ Missing Tier 1 keyword: '{keyword}' ({category})")
        
        total = len(integrated) + len(missing)
        integration_rate = len(integrated) / total if total > 0 else 0.0
...
"""
    ...
    def _validate_keyword_integration(self, tailored_data: Dict[str, Any], recommendations: RecommendationAnalysis, request_id: str = 'debug') -> None:
        ...
        # Require at least 60% of keywords to be integrated (40% missing allowed for flexibility)
        if missing_keywords and len(missing_keywords) > len(valid_keywords) * 0.4:
            missing_list = ", ".join(missing_keywords)
            logger.warning(f"⚠️ Many keywords not found but continuing: {missing_list}")
