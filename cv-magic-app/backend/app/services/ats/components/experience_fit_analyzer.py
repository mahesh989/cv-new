...
class ExperienceFitAnalyzer:
    ...
    def _parse_response(self, raw_response: str) -> Dict[str, Any]:
        ...
        logger.error("[EXP_FIT_ANALYZER] Failed to parse LLM response. Raw: %s", raw_response[:400])
        raise ValueError("Experience & Fit analysis response not valid JSON")
    ...
    async def analyze(
        ...
        Returns:
            Dict containing experience & fit analysis results
        """
        ...
        logger.info("[EXP_FIT_ANALYZER] Requesting unified experience & fit analysis...")
        try:
            ...
            logger.error("[EXP_FIT_ANALYZER] Analysis failed: %s", str(e))
            raise ValueError(f"Experience & Fit analysis failed: {str(e)}")
    ...
    def _validate_result(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key in required_keys:
            if key not in result:
                logger.warning(f"[EXP_FIT_ANALYZER] Missing key: {key}, adding default")
                result[key] = {
                    "score": 50,
            ...
            if not isinstance(score, (int, float)) or score < 0 or score > 100:
                logger.warning(f"[EXP_FIT_ANALYZER] Invalid score for {key}: {score}, capping to 0-100")
