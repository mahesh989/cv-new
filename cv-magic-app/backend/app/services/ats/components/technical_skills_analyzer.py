...
class TechnicalSkillsAnalyzer:
    ...
    def _parse_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            pass
        # Find JSON boundaries again from raw
        start = response.find("{")
        ...
        if start != -1 and end > start:
            try:
                ...
                pass
        logger.error("[TECH_SKILLS_ANALYZER] Failed to parse LLM response. Raw: %s", response[:400])
        raise ValueError("Unified technical & skills analysis JSON parsing failed")
    ...
    def _validate_result(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key in required_keys:
            if key not in result:
                logger.warning("[TECH_SKILLS_ANALYZER] Missing key: %s, adding default", key)
                result[key] = {"score": 50}
    ...
    async def analyze(
        ...
        """
        try:
            logger.info("[TECH_SKILLS_ANALYZER] Starting unified technical & skills analysis")

            cv_text_truncated = cv_text[:5000] if cv_text else ""
            jd_text_truncated = jd_text[:3000] if jd_text else ""

            formatted_prompt = self.prompt_template.format(
                cv_text=cv_text_truncated, jd_text=jd_text_truncated, matched_skills=matched_skills or "[]"
            )
            ...
            logger.error("[TECH_SKILLS_ANALYZER] Analysis failed: %s", str(e), exc_info=True)
            raise
